...
The cream device is an interactive browser representation of the Waveshare ESP32-S3-Touch-AMOLED-2.16 playground. Its screen layout follows the firmware's stage, mood picker, and character picker, while the front-facing shell uses HTML and CSS. The website draws the character locally using Canvas 2D and JavaScript. It does not flash or control your board. Speaking animates a mouth; no audio is played, no microphone is requested, and no speech recognition runs. For device installation, use the [flashing guide](/docs/flashing/).
